...
/**
 * Returns an array containing the results of calling the supplied block on each
 * object in the array. The block calls are performed concurrently, but the
 * order is preserved.
 *
 ...
 *
 * @result Array containing the ordered result of each block call.
 */
 ...
 * Returns an array containing the results of calling the supplied block on each
 * object in the array. The block calls are performed concurrently, but the
 * order is preserved. The block calls are dispatched on the supplied concurrent
 * dispatch queue.
 *
...
/**
 * Returns a subsequence of the array, containing only the objects for which the
 * predicate block returns true. The block calls are performed concurrently, but
 ...
 *
 * @result Array containing only the objects for which the predicate block returned true.
 */
 ...
 * Returns a subsequence of the array, containing only the objects for which the
 * predicate block returns true. The block calls are performed concurrently, but
 * the order is preserved. The block calls are dispatched on the supplied
